...
def get_translated_optimal_query(query: str) -> str:
    """
    生成翻译后的优化检索词（与原query不同语言）
    ...
    target_language = "中文" if is_english else "英文"

    prompt = f"""
你是一名专业的社会科学研究人员，请为<用户查询文本>生成2到4个语言为{target_language}的文献检索单词，在生成过程中，严格遵守<检索词要求>，并且按照<输出结构>输出翻译后的查询文本。

<检索词要求>
1. 提取查询中的专业词汇，包括理论名称、学术概念、研究方法作为检索词
2. 检索词表达为{target_language}时，应当立足于学术场景，请勿直译
3. 检索词之间采用空格连接，不要包含逗号等标点符号
